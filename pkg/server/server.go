...
func New(db *gendb.Queries, log *slog.Logger, v *validator.Validate) *gin.Engine {
	r := gin.Default()

	g1 := r.Group("/api", middleware.Validator(v), middleware.Log(log), middleware.Database(db))
	g1.GET("/logout", auth.LogoutHandler)
	g1.POST("/login", auth.LoginHandler)
	g1.POST("/users", user.CreateHandler)

	g2 := r.Group("/api", middleware.Validator(v), middleware.Log(log), middleware.Database(db), middleware.Authentication(db), middleware.HandlerAuthorization(db))
	...
	g3 := r.Group("/api", middleware.Validator(v), middleware.Log(log), middleware.Database(db), middleware.Authentication(db), middleware.Authorization(db, types.RoleAdmin))
	g3.DELETE("/roles/:uid/:rid", role.RemoveFromUserHandler)
	g3.POST("/roles", role.AddToUserHandler)

	g4 := r.Group("/api", middleware.Validator(v), middleware.Log(log), middleware.Database(db), middleware.Authentication(db), middleware.Authorization(db, types.RoleAdmin, types.RoleTeacher))
